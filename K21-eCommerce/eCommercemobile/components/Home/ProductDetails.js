...
import { ActivityIndicator, Animated, KeyboardAvoidingView, Image, SafeAreaView, ScrollView, Text, TextInput, TouchableOpacity, TouchableWithoutFeedback, View, useWindowDimensions, Platform, Alert } from "react-native";
...
const ProductDetails = ({ route, navigation }) => {
    ...
    const loadProducts = async () => {
        try {
            ...
            let productList = Array.isArray(res.data.results) ? res.data.results : [res.data.results];
    ...
    const handleBuyPress = async () => {
        try {
            let token = await AsyncStorage.getItem('access-token');
            let res = await authApi(token).post('https://tuankiet.pythonanywhere.com/carts/1/add_to_cart/', {
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor:"white", alignItems:"center"}}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
                <ScrollView onScroll={loadMoreInfo} style={{ marginBottom: 50 }}>                  
                    ...
                    <Card style={{ margin: 10, padding:5, backgroundColor:"white" }}>
                        <Text style={{ fontSize: 20, fontWeight: "bold", color:"black"}}>So sánh sản phẩm cùng loại</Text>
                        <RNPickerSelect
                        ...
                        />
                        <TouchableOpacity style={[MyStyles.button, {backgroundColor:"red"}]} onPress={handleComparePress}>
                            <Text style={MyStyles.buttonText}>So sánh</Text>
                        </TouchableOpacity>
                    </Card>
                    ...
                    {showComments && comments !== null && (
                        <>
                            ...
                            {user === null || user.role !== 'BUYER' ? "" : <>
                                <View style={[MyStyles.row, {alignItems:"center", justifyContent:"center"}]}>
                                    <TextInput value={contentComment} onChangeText={t => setContentComment(t)} style={{ height:40, width: 290, backgroundColor: "lightgray", padding: 5, margin:5 }} placeholderTextColor="gray"  placeholder="Nội dung bình luận..." />
                            ...
                            {editingCommentId !== null && user.role === 'BUYER' && (
                            <View>
                                <Portal>
                            ...
                            {deletingCommentId !== null && user.role === 'BUYER' && (
                            <View>
                                <Portal>
                            ...
                            {comments.map((c) => (
                                <View key={c.id}>
                                    <List.Item
                                    style={{ borderColor:"lightgray", borderWidth:1, borderRadius:10, height:100, justifyContent:"center", alignItems:"center", marginVertical:5, flexDirection:"row" }}
                                    ...
                                    right={() => 
                                        <View>
                                            ...
                                            <View>
                                                <TouchableOpacity onPress={() => handleDelete(c.id,'comment')} style={{ marginLeft: 5, padding:2 }}>
                                                ...
                                                <TouchableOpacity onPress={() => setReplyingCommentId(c.id)} style={{ marginLeft: 5, padding:2 }}>
                                                    <Text style={{ color: 'green' }}>Phản hồi</Text>
                                                </TouchableOpacity>
                                            </View>
                                        )}
                                            {user && user.id !== c.buyer.id && user.role === 'BUYER' && (
                                                <TouchableOpacity onPress={() => setReplyingCommentId(c.id)} style={{ marginLeft: 10, padding:2 }}>
                                                    <Text style={{ color: 'green' }}>Phản hồi</Text>
                                                </TouchableOpacity>
                                            )}
                                        </View>
                                    ...
                                    title={
                                    <View>
                                        <Text style={{ fontSize: 16, fontWeight:"bold", marginRight:80}}>{c.buyer.username}</Text>                                       
                                    </View>}
                                    description={
                                        <View>
                                            <Text>{c.content}</Text>
                                    ...
                                    {replyingCommentId === c.id && (
                                        <View style={MyStyles.row}>
                                            <TextInput
                                                ...
                                                style={{ height:45, width: "100%", backgroundColor: "lightgray", padding: 5}}
                                                placeholder="Nội dung phản hồi"
                                                placeholderTextColor="gray"
                                            />
                                            <TouchableOpacity onPress={() => addReply(c.id)}>
                                                <Text style={{textAlign: "center", backgroundColor: '#ff4500', color: "white", width:80, height:40, padding:10, marginTop: 5}}>Gửi</Text>
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={() => setReplyingCommentId(null)}>
                                                <Text style={{textAlign: "center", backgroundColor: 'white', color: "#ff4500", marginTop: 5, marginLeft:2, width:80, height:40, padding:10, borderWidth:2, borderColor:"lightgray"}}>Hủy</Text>
                                            </TouchableOpacity>
                                        </View>
                                    )}
                                    ...
                                    {c.replies && c.replies.map((reply) => (
                                        <List.Item
                                            ...
                                            right={() => 
                                                <View>
                                                    {user && user.id === reply.buyer.id && user.role === 'BUYER' && (
                                                    <View>
                                                        <TouchableOpacity onPress={() =>  handleDelete(reply.id,'comment')} style={{ marginLeft: 5, padding:2 }}>
                                                            <Text style={{ color: 'red' }}>Xóa</Text>
                                                        </TouchableOpacity>
                                                        <TouchableOpacity onPress={() => {
                    ...
                    {showRatings && ratings !== null && (
                        <View>
                            ...
                            {deletingRatingId !== null && user.role === 'BUYER' && (
                            <View>
                                <Portal>
                                <Dialog visible={dialogVisible2} onDismiss={closeDialog2} style={{ backgroundColor:"white"}}>
                            ...
                            {ratings.map((r) => (
                                <View key={r.id}>
                                    {editingRatingId === r.id ? (
                                        ...
                                        <List.Item
                                            style={{ borderColor:"lightgray", borderWidth:1, borderRadius:10, height:80, justifyContent:"center", marginVertical:5 }}
